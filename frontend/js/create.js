...
function openClassPopup(playerClass) {
  ...
  document.getElementById('popup-class-label').textContent = {
    warrior: 'Maitre du combat rapproche',
    mage:    'Tisseur arcanes devastateur',
    archer:  'Precision mortelle a distance',
    rogue:   'Rapide et insaisissable',
  }[playerClass];
  document.getElementById('popup-stats').innerHTML = `
    <div class="popup-stat"><span class="label">HP</span><span class="value">${s.hp}</span></div>
    <div class="popup-stat"><span class="label">Energie</span><span class="value">${s.energy}</span></div>
    <div class="popup-stat"><span class="label">Force</span><span class="value">${s.strength}</span></div>
    <div class="popup-stat"><span class="label">Agilite</span><span class="value">${s.agility}</span></div>
  `;
...
btnNameNext.addEventListener('click', async () => {
  ...
  if (val.length < 3) { nameError.textContent = 'Au moins 3 caracteres'; return; }
  if (val.length > 20) { nameError.textContent = 'Maximum 20 caracteres'; return; }
...
function renderConfirm() {
  ...
  document.getElementById('confirm-stats').innerHTML = `
    <div class="confirm-stat"><span class="label">HP</span><span class="value">${s.hp}</span></div>
    <div class="confirm-stat"><span class="label">Energie</span><span class="value">${s.energy}</span></div>
    <div class="confirm-stat"><span class="label">Force</span><span class="value">${s.strength}</span></div>
    <div class="confirm-stat"><span class="label">Agilite</span><span class="value">${s.agility}</span></div>
...
document.getElementById('btn-confirm-create').addEventListener('click', async () => {
  ...
  try {
    ...
    if (res.status === 409) {
      createError.textContent = 'Ce nom est deja pris.';
      showScreen('screen-name');
      return;
    }
    ...
    btn.disabled = false;
    btn.textContent = 'Commencer aventure';
  }
});
...
document.getElementById('btn-play').addEventListener('click', () => {
  alert('Exploration - a venir dans etape 3 !');
});
